...
def test_sparse_generator_with_level_free_03():
    ...
    generate = sparse_rail_generator(max_num_cities=5, max_rails_between_cities=3, grid_mode=True, p_level_free=0.3)
    _, agent_hints = generate(width=25, height=30, num_agents=1, np_random=RandomState(215545))
    assert agent_hints["level_free_positions"] == set()
...
def test_sparse_generator_with_level_free_04():
    ...
    rail, agent_hints = generate(width=25, height=30, num_agents=1, np_random=RandomState(215545))
    assert agent_hints["level_free_positions"] == {(18, 20)}
    for cell in agent_hints["level_free_positions"]:
...
def test_sparse_generator_with_level_free_09():
    ...
    rail, agent_hints = generate(width=25, height=30, num_agents=1, np_random=RandomState(215545))
    assert agent_hints["level_free_positions"] == {(4, 12), (18, 20)}
    for cell in agent_hints["level_free_positions"]:
        assert rail.grid[cell] == RailEnvTransitionsEnum.diamond_crossing
