...
def test_shortest_path_policy_runs_shuttle_trains_exactly_on_timetable():
    """
    Three stations A=(0,1), B=(0,4), C=(0,7) on one straight track, 3 cells apart. Four trains, each at
    max speed 1, run one leg each: two shuttle A<->B (legs 0: A->B, 2: B->A), two shuttle B<->C (legs 1:
    B->C, 3: C->B). Their earliest departures are staggered by a fixed headway of 6 steps, round-robin
    over the four legs (leg *h* departs at `(h+1) * HEADWAY`), so no two trains ever need station B - the
    only cell shared between the two routes - at the same time.

    - A `ShortestPathPolicy` alone drives every train from its start waypoint to its target waypoint,
      one cell per step (speed 1, no malfunctions, no other traffic in the way).
    - Each train stays off the track until its own earliest departure, appears at its start station on
      exactly that step, advances by exactly one cell per following step, and reaches its target -
      leaving the track immediately - exactly `STATION_DISTANCE` steps after departing, matching its
      timetabled latest arrival exactly.
    - No train is ever stopped: the timetable's headway leaves enough clearance that the four trains
      never contend for a cell.
    """
    rail = _make_three_station_rail()
    env = RailEnv(width=N_CELLS, height=1,
    ...
    # verbatim expected timetable: one entry per leg, in handle order 0..3 (A->B, B->C, B->A, C->B),
    # departures at HEADWAY=6, 12, 18, 24 and arrivals STATION_DISTANCE=3 steps later.
    assert _timetable_generator(env.agents, env.distance_map, None, None) == Timetable(
    ...
    position_by_step: Dict[int, Dict[int, Tuple[int, int]]] = {handle: {} for handle in LEG_WAYPOINTS}
    done = False
    while not done:
        actions = policy.act_many(env.get_agent_handles(), observations=list(obs.values()))
        obs, _, dones, _ = env.step(actions)
        for handle, agent in enumerate(env.agents):
            position_by_step[handle][env._elapsed_steps] = agent.current_entry_point[0] if agent.current_entry_point is not None else None
            assert agent.state != TrainState.STOPPED
        done = dones['__all__']

    # verbatim expected position trace: for each leg (dict key), its position at every elapsed step
    # from 1 to 27 - `None` off track, `(0, col)` on track, `None` again once removed on arrival.
    assert position_by_step == {
    ...
    for handle, leg in LEG_WAYPOINTS.items():
        ...
        for step in range(1, departure_step):
            assert position_by_step[handle][step] is None, f"leg {handle}: on track before its earliest departure at step {step}"
        for offset in range(STATION_DISTANCE):
            ...
            assert position_by_step[handle][step] == expected_position, f"leg {handle} at step {step}"
        assert position_by_step[handle][arrival_step] is None, f"leg {handle}: still on track at its latest arrival step {arrival_step}"
