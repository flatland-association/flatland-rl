...
def _step_along_shortest_path(env, obs_builder, rail):
    ...
    for agent in env.agents:
        ...
        for exit_direction in range(4):
            neighbour = get_new_position(agent.current_entry_point[0], exit_direction)
                ...
                for agent_orientation in range(4):
                    ...
                    if is_valid:
                        distance_to_target = obs_builder.env.distance_map.get()[
                            (agent.handle, *agent.current_entry_point[0], exit_direction)]
                        print("agent {} at {} facing {} taking {} distance {}".format(agent.handle,
                                                                                      agent.current_entry_point[0],
                                                                                      agent.current_entry_point[1],
                                                                                      exit_direction,
                                                                                      distance_to_target))
                        ...
                        if distance_to_target < shortest_distance:
                            ...
                            agent_dir_idx = (exit_direction - agent.current_entry_point[1]) % num_directions
...
def test_reward_function_conflict(rendering=False):
    ...
    agent = env.agents[0]
    agent.current_entry_point = ((5, 6), 0)  # south dead-end, facing north
    agent.initial_entry_point = ((5, 6), 0)  # south dead-end, facing north
    agent.targets = {((3, 9), d) for d in Grid4TransitionsEnum}  # east dead-end
    agent.moving = True
    agent._set_state(TrainState.MOVING)

    agent = env.agents[1]
    agent.current_entry_point = ((3, 8), 3)  # east dead-end, facing west
    agent.initial_entry_point = ((3, 8), 3)  # east dead-end, facing west
    ...
    while iteration < 5:
        ...
        for agent in env.agents:
            ...
            assert agent.current_entry_point[0] == expected_position, \
                "[{}] agent {} at {}, expected {}".format(iteration + 1, agent.handle,
                                                          agent.current_entry_point[0], expected_position)
        if rendering:
            renderer.render_env(show=True, show_observations=True)
    ...
    while iteration < 7:
        ...
        for agent in env.agents:
            print("[{}] agent {} at {}, target {} ".format(
                iteration + 1, agent.handle, agent.current_entry_point[0], agent.targets))
        print(np.all([np.array_equal(agent2.current_entry_point[0], next(iter(agent2.targets))[0])
