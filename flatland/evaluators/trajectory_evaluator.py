...
class TrajectoryEvaluator:
    ...
    def evaluate(
        ...
        self.trajectory.build_cache()

        done = False
        for elapsed_before_step in tqdm.tqdm(range(start_step, end_step), **tqdm_kwargs):
            action = {agent_id: self.trajectory.action_cache[elapsed_before_step].get(agent_id, RailEnvActions.MOVE_FORWARD) for agent_id in range(n_agents)}
            ...
            for agent_id in range(n_agents):
                ...
                expected_position = self.trajectory.position_cache[elapsed_after_step][agent_id]
                ...
                if not skip_rewards_dones_infos:
                    ...
                    expected_reward, expected_done, expected_info = self.trajectory.trains_rewards_dones_infos_cache[elapsed_after_step][agent_id]
                    if not skip_rewards:
