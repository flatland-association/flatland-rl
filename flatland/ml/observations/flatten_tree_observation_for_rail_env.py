...
class FlattenedTreeObsForRailEnv(GymObservationBuilder[RailEnv, np.ndarray], TreeObsForRailEnv):
    ...
    def split_tree_into_feature_groups(self, root_node: Node, max_tree_depth: int) -> (np.ndarray, np.ndarray, np.ndarray):
        """

        This function splits the tree into three difference arrays of values:

        - `data` (6 features per node):
        ...
        """
        data, distance, agent_data = self._split_node_into_feature_groups(root_node)

        for direction in TreeObsForRailEnv.tree_explored_actions_char:
            sub_data, sub_distance, sub_agent_data = self._split_subtree_into_feature_groups(root_node.childs[direction], 1,
                                                                                             max_tree_depth)
            data = np.concatenate((data, sub_data))
            distance = np.concatenate((distance, sub_distance))
            agent_data = np.concatenate((agent_data, sub_agent_data))
