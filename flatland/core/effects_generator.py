...
class EffectsGenerator(Generic[EnvironmentType]):
    ...
    def __init__(self, hook: Callable[[EnvironmentType], EnvironmentType] = None):
        self._hook = hook

    def __call__(self, env: EnvironmentType, *args, **kwargs) -> EnvironmentType:
        if self._hook is None:
            return env
        return self._hook(*args, **kwargs)


def effects_generator_wrapper(*post_hooks: EffectsGenerator[EnvironmentType]):
