...
class SparseLineGen(BaseLineGen):
    ...
    def generate(self, rail: RailGridTransitionMap, num_agents: int, hints: dict = None, num_resets: int = 0, np_random: RandomState = None) -> Line:
        ...
        for agent_id, agent_waypoints_ in agent_waypoints.items():
            ...
            # new_agent_waypoints[agent_id][-1] = [Waypoint(new_agent_waypoints[agent_id][-1][0].position, d) for d in Grid4TransitionsEnum]
...
def line_from_file(filename: Union[str, Path] = None, load_from_package=None, env_dict: dict = None) -> LineGenerator:
    """
    Utility to load line_generator from persisted env - uses env_dict if populated, otherwise tries to load from file / package.
