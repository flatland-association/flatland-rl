...
class TreeObsStandardizedForRailEnv(ObservationBuilder):
    """
    TreeObsForRailEnv object.
...
class GlobalObsStandardizedForRailEnv(ObservationBuilder):
    """
    Gives a global observation of the entire rail environment.
    The observation is composed of the following elements:
    ...
    def __init__(self):
        super(GlobalObsStandardizedForRailEnv, self).__init__()
    ...
    def get(self, handle: int = 0) -> (np.ndarray, np.ndarray, np.ndarray):
        ...
        return self._transform_state((self.rail_obs, obs_agents_state, obs_targets), self.env.width, self.env.height)

    def _transform_state(self, obs, env_width, env_height):
        return GlobalObsStandardizer().standardize_global_observation(obs[0], obs[1], obs[2], env_width, env_height)

class LocalObsStandardizedForRailEnv(ObservationBuilder):
    """
    !!!!!!WARNING!!! THIS IS DEPRACTED AND NOT UPDATED TO FLATLAND 2.0!!!!!
    Gives a local observation of the rail environment around the agent.
    The observation is composed of the following elements:
    ...
    def __init__(self, view_width, view_height, center):
        ...
        super(LocalObsStandardizedForRailEnv, self).__init__()
        self.view_width = view_width
        self.view_height = view_height
        self.center = center
        self.max_padding = max(self.view_width, self.view_height - self.center)
    ...
    def get(self, handle: int = 0) -> (np.ndarray, np.ndarray, np.ndarray, np.ndarray):
        ...
        direction = np.identity(4)[agent.direction]
        return self._transform_state((local_rail_obs, obs_map_state, obs_other_agents_state, direction))

    def _transform_state(self, obs):
        view_size = self.view_height * (2*self.view_width+1)
        return LocalObsStandardizer().standardize_local_observation(obs[0], obs[1], obs[2], obs[3], view_size)
