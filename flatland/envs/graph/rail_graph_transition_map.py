...
* An edge is defined by "from-node" u and "to-node" v such that for the edge e = (u, v).  Edges reflect feasible transition from node u to node v. We can think of the suggestive notation :math:`[u,v)` in terms of resource occupation of the underlying cell, as the "from-node" refers to the underlying cell entered, and the "to-node" refers to the neighbor cell entered when the edge is left.
...
class GraphTransitionMap(TransitionMap[GridNode, GridEdge, bool, RailEnvActions]):
    ...
    def grid_to_digraph(transition_map: GridTransitionMap) -> nx.DiGraph:
        ...
        prohibited_actions : Set[str]
            set of actions that must not be chosen when passing this node

        Prohibited actions and all edge actions must be pairwise disjoint and cover all 5 Flatland actions.
            ...
            for c in range(transition_map.width):
                for d in range(4):
                    ...
                    for a in RailEnvActions:
                        t = transition_map.apply_action_independent(a, ((r, c), d))
                        if t is None:
