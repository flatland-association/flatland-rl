...
class NetworkImpactPropagation(Rewards[Tuple[float, int]]):
    """
    Network Impact Propagation:  1 - #(agents with delay or agents with stops not served) / #agents.
    Agents with delay or agents with stops not served is the same as agents not punctual.

    The implementation returns the tuple `1 - #(agents with delay or agents with #stops not served) / #agents, #agents`.
    """

    def __init__(self):
        self.punctuality = PunctualityRewards()

    def step_reward(self, agent: EnvAgent, agent_transition_data: AgentTransitionData, distance_map: DistanceMap, elapsed_steps: int) -> Tuple[float, int]:
        return self.punctuality.step_reward(agent, agent_transition_data, distance_map, elapsed_steps)

    def end_of_episode_reward(self, agent: EnvAgent, distance_map: DistanceMap, elapsed_steps: int) -> Tuple[int, int]:
        n_stops_on_time, n_stops = self.punctuality.end_of_episode_reward(agent, distance_map, elapsed_steps)
        if n_stops_on_time == n_stops:
            return 1, 1
        else:
            return 0, 1

    def cumulate(self, *rewards: Tuple[float, int]) -> Tuple[float, int]:
        num_agents_not_punctual = sum([(1.0 - r[0]) * r[1] for r in rewards])
        return num_agents_not_punctual, sum([r[1] for r in rewards])

    def empty(self) -> Tuple[float, int]:
        return 0, 0
