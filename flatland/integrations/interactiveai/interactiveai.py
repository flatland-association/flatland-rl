...
class FlatlandInteractiveAICallbacks(FlatlandCallbacks):
    ...
    def on_episode_step(
        ...
        **kwargs,
        ...
        for agent in env.agents:
            if agent.current_entry_point is not None and agent.current_entry_point[0] not in self.coordinate_map:
                warnings.warn(f"Missing mapping for {agent.current_entry_point[0]}")
        context = {
            "use_case": "Railway",
            "data": {
                "trains": [
                    {
                        ...
                        'latitude': f"{self.coordinate_map[agent.current_entry_point[0]][0]}",
                        'longitude': f'{self.coordinate_map[agent.current_entry_point[0]][1]}',
                    } for agent in env.agents if agent.current_entry_point is not None
                                                 # TODO
                                                 and agent.current_entry_point[0] in self.coordinate_map]
            }
        }
